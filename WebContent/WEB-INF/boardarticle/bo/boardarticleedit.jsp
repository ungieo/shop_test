...
<script type="text/javascript">
	...
	function dataUp(){
		$('#r_bdacontent').val(CKEDITOR.instances.editor1.getData());
				...
				<div class="">
					...
					<div class="row">
						<div class="col-md-12 col-sm-12 col-xs-12">
							<div class="x_panel">
								<div class="x_title">
									<h2>게시글 수정 <small>Board Edit</small></h2>
...
<!--                        				</ul> -->
								...
								<div class="x_content">
									...
									<form action="boardarticleup" class="form-horizontal" id="frm" name="frm" method="post" enctype="multipart/form-data" >
									
										<input name="r_bdaseq" type="hidden" value="${ model.boardarticle.BDA_SEQ }" />
										<input name="r_bdacontent" id="r_bdacontent" type="hidden" />
									
										<fieldset>
<!-- 											<legend>boardarticleadd</legend> -->
												...
												<div class="col-sm-10">
													<select class="form-control" name="r_bdabdid">
														<c:forEach var="board" items="${ model.boardList }">
															<option value="${board.BD_ID }" <c:if test="${ board.BD_ID eq model.boardarticle.BDA_BDID }">selected="selected"</c:if> >${board.BD_NAME }</option>
														</c:forEach>
													</select>
<%-- 													<input id="r_bdabdid" name="r_bdabdid" placeholder="게시판아이디" class="form-control" type="text" value="${ model.boardarticle.BDA_BDID }" required /> --%>
												</div>
											</div>
											<div class="form-group">
											...
											<div class="form-group">
												...
												<div class="col-sm-10">
													<textarea class="form-control" id="editor1" name="editor1" placeholder="내용">${ model.boardarticle.BDA_CONTENT }</textarea>
												</div>
											</div>
											<div class="form-group">
												<label class="col-sm-2 3 col-xs-12 control-label" for="r_bdafilenum">파일첨부</label>
												...
												<div class="col-sm-3 3 col-xs-12">
											...
											<div class="form-group">
												...
												<div class="col-sm-10">
													<input id="r_bdambid" name="r_bdambid" placeholder="작성자아이디" class="form-control" type="text" value="${ model.boardarticle.BDA_MBID }" />
												</div>
											</div>
											<div class="form-group">
												<label class="col-sm-2 control-label" for="r_bdaownername">작성자명</label>
												<div class="col-sm-10">
													<input id="r_bdaownername" name="r_bdaownername" placeholder="작성자명" class="form-control" type="text" value="${ model.boardarticle.BDA_OWNERNAME }" />
												</div>
											</div>
											<div class="form-group">
...
<%-- 												<input id="r_bdastatus" name="r_bdastatus" placeholder="상태값" class="form-control" type="text" value="${ model.boardarticle.BDA_STATUS }" /> --%>
												<label class="radio-inline"><input name="r_bdastatus" type="radio" value="0" <c:if test="${ model.boardarticle.BDA_STATUS eq 0 }">checked="checked"</c:if> />일반</label>
											</div>
										</div>
										<div class="form-group">
...
<%-- 												<input id="r_bdalevel" name="r_bdalevel" placeholder="레벨" class="form-control" type="text" value="${ model.boardarticle.BDA_LEVEL }" /> --%>
												<label class="radio-inline"><input name="r_bdalevel" type="radio" value="N" <c:if test="${ model.boardarticle.BDA_LEVEL eq 'N' }">checked="checked"</c:if> />일반</label>
											</div>
										</div>
										<div class="form-group">
...
<%-- 												<input id="r_bdatype" name="r_bdatype" placeholder="타입" class="form-control" type="text" value="${ model.boardarticle.BDA_TYPE }" /> --%>
												<label class="radio-inline"><input name="r_bdatype" type="radio" value="N" <c:if test="${ model.boardarticle.BDA_TYPE eq 'N' }">checked="checked"</c:if> />일반</label>
											</div>
										</div>
										<div class="form-group">
											<label for="r_bdause" class="col-sm-2 control-label">사용여부</label>
											<div class="col-sm-10">
												<label class="radio-inline"><input name="r_bdause" type="radio" value="Y" <c:if test="${ model.boardarticle.BDA_USE eq 'Y' }">checked="checked"</c:if> />YES</label>
												<label class="radio-inline"><input name="r_bdause" type="radio" value="N" <c:if test="${ model.boardarticle.BDA_USE eq 'N' }">checked="checked"</c:if> />NO</label>
											</div>
										</div>
