...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			//---글 저장이 아닐 때 enctype을 변경해야함. 물론 a를 사용하면 되지만..
			$('#frm').submit(function(event){
				if( 'boardarticlein' != $(this).attr('action') ){
					$(this).attr('enctype', '');
// 					$(this)[0].submit();
					$(this).off().submit();
				}
			...
			$('#frm').validate({
				rules: {
					r_bdaname: { required: true, minlength: 2, },
					r_bdacontent:{ required: true, minlength: 5 },
					r_bdapswd: { required: true, minlength: 4 }
				}
                    ...
                    <div class="box">
						...
						<form action="boardarticlein" class="form-horizontal" id="frm" name="frm" method="post" enctype="multipart/form-data">
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<input class="form-control" id="r_bdaname" name="r_bdaname" placeholder="제목" type="text" value="${ model.boardarticle.BDA_NAME }" />
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label" for="editor1">내용</label>
								<div class="col-sm-10">
									<textarea class="form-control" id="editor1" name="editor1" placeholder="내용" ></textarea>
								</div>
							</div>
							...
							<c:if test="${ 'Y' eq model.board.BD_FILEUSE }">
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input class="form-control" id="r_bdafilenum" name="r_bdafilenum" placeholder="파일개수" type="file" value="${ model.boardarticle.BDA_FILENUM }" />
									</div>
								</div>
							</c:if>
							...
							<c:if test="${ 'Y' eq model.board.BD_PSWDUSE }">
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input class="form-control" id="r_bdapswd" name="r_bdapswd" placeholder="비밀문자" type="password" value="${ model.boardarticle.BDA_PSWD }" />
									</div>
								</div>
							</c:if>
